...
void nhapMotVatTu(VatTu **ds, int pos, int n);
...
void xuatHangHoa(VatTu **ds, int n);
...
{
    if (a.ngay < 10)
        fprintf(f, "0%d/", a.ngay);
    else
        fprintf(f, "%d/", a.ngay);
    if (a.thang < 10)
        fprintf(f, "0%d/", a.thang);
    else
        fprintf(f, "%d/", a.thang);
    fprintf(f, "%-7d", a.nam);
}
...
void nhapMotVatTu(VatTu **ds, int pos, int n)
{
    ...
    do
    {
        trung = 0;
        printf("Ma vat tu: ");
        scanf("%9s", (*ds)[pos].ma_vat_tu);
        for (i = 0; i < n; i++)
        {
            if ((i != pos) && strcmp((*ds)[pos].ma_vat_tu, (*ds)[i].ma_vat_tu) == 0)
            {
    ...
    scanf(" %19[^\n]", (*ds)[pos].loai_vat_tu);
    printf("Don vi tinh: ");
    scanf("%9s", (*ds)[pos].don_vi_tinh);
    printf("Ngay nhap (dd/mm/yyyy): ");
    nhapNgay(&(*ds)[pos].ngay_nhap);
    printf("Nha san xuat: ");
    scanf(" %49[^\n]", (*ds)[pos].nha_san_xuat);
    printf("So luong: ");
    scanf("%d", &(*ds)[pos].so_luong);
    printf("Don gia: ");
    scanf("%f", &(*ds)[pos].don_gia);
}
...
{
    inGioiThieu();
    printf("Sap xep danh sach vat tu theo:\n");
    printf("0. Thoat\n");
    printf("1. Ma vat tu\n");
    printf("2. Ten vat tu\n");
    printf("3. Ngay nhap\n");
...
void themVatTuVaoCuoi(VatTu **ds, int *n)
{
    ...
    do
    {
        ...
        printf("\nThem thanh cong!\n");
        printf("Ban co muon nhap tiep khong? (1: Co, 0: Khong): ");
        scanf("%d", &tiepTuc);
...
{
    setColor(10);
    VatTu *temp = malloc(sizeof(VatTu));
    int count = 0;
    temp[0].ma_vat_tu[0] = '\0'; // Khoi tao ma vat tu rong
    int chon = 0;
    do
    {
        printf("Nhap ma vat tu can xuat: ");
        scanf("%s", temp[count].ma_vat_tu);
        int i;
        for (i = 0; i < n; i++)
            if (strcmp(temp[count].ma_vat_tu, (*ds)[i].ma_vat_tu) == 0)
                break;
        if (i < n)
        {
            temp[count] = (*ds)[i];
            ...
            xuatMotVatTu(temp[count]);
            setColor(10);
            printf("Nhap so luong can xuat: ");
            scanf("%d", &temp[count].so_luong);
            while (temp[count].so_luong > (*ds)[i].so_luong || temp[count].so_luong < 0)
            {
                printf("So luong xuat khong hop le. So luong hien co: %d\n", (*ds)[i].so_luong);
                printf("Nhap lai so luong can xuat: ");
                scanf("%d", &temp[count].so_luong);
            }
            (*ds)[i].so_luong = (*ds)[i].so_luong - temp[count].so_luong;
            printf("Xuat thanh cong %d %s\n", temp[count].so_luong, temp[count].ten_vat_tu);
            tinhThanhTien(&temp[count]);
            tinhThanhTien(&(*ds)[i]);
            ++count;
            resizeArray(&temp, count + 1);
        }
        else
        {
            printf("Khong tim thay vat tu can xuat.\n");
        }
        printf("Ban co muon xuat vat tu khac khong? (1: Co, 0: Khong): ");
        scanf("%d", &chon);
    } while (chon == 1);
    if (count > 0)
    {
        int tongSo = 0;
        float tongTien = 0.0;
        printf("Danh sach vat tu da xuat:\n");
        setColor(14);
        printf("%-10s %-20s %-15s %-10s %-12s %-15s %-10s %-10s %-10s\n",
               "Ma VT", "Ten VT", "Loai VT", "DVT", "Ngay Nhap", "Nha SX", "So Luong", "Don Gia", "Thanh Tien");
        int i;
        ...
        {
            xuatMotVatTu(temp[i]);
            tongSo += temp[i].so_luong;
            tongTien += temp[i].thanh_tien;
        }
        setColor(10);
        printf("Tong so luong hang hoa da xuat: %d\n", tongSo);
        printf("Tong tien hang hoa da xuat: %.2f\n", tongTien);
        printf("Xuat hoa don ra file? (1: Co, 0: Khong): ");
        scanf("%d", &chon);
        if (chon > 0)
        {
            char filename[100];
            do
            {
                printf("Nhap ten file can xuat hoa don(co phan mo rong .txt): ");
                scanf("%s", filename);
                FILE *f = fopen(filename, "w");
                if (f == NULL)
                {
                    printf("Khong mo duoc file. Vui long chon.\n");
                    printf("0. Thoat\n");
                    printf("1. Nhap lai ten file\n");
                    printf("Chon: ");
                    scanf("%d", &chon);
                }
                else
                    chon = 0; // Da mo duoc file
            } while (chon > 0);
            int i;
            FILE *f = fopen(filename, "w");
            fprintf(f, "Danh sach vat tu da xuat:\n");
            fprintf(f, "%-10s %-20s %-15s %-10s %-12s %-15s %-10s %-10s %-10s\n",
                    "Ma VT", "Ten VT", "Loai VT", "DVT", "Ngay Nhap", "Nha SX", "So Luong", "Don Gia", "Thanh Tien");
            for (i = 0; i < count; i++)
            {
                fprintf(f, "%-10s %-20s %-15s %-10s ", temp[i].ma_vat_tu, temp[i].ten_vat_tu, temp[i].loai_vat_tu, temp[i].don_vi_tinh);
                xuatNgay(temp[i].ngay_nhap, f);
                fprintf(f, "%-15s %-10d %-10.2f %-10.2f\n",
                        temp[i].nha_san_xuat, temp[i].so_luong, temp[i].don_gia, temp[i].thanh_tien);
            }
            fprintf(f, "Tong so luong hang hoa da xuat: %d\n", tongSo);
            fprintf(f, "Tong tien hang hoa da xuat: %.2f\n", tongTien);
            fclose(f);
        }
    }
    else
    {
        setColor(10);
        printf("Khong xuat hhang hoa nao.\n");
    }
    free(temp);
}
...
void inGioiThieu()
{
    ...
    printf("| Sinh vien thuc hien: Duong Sy An       MSSV: 102240014           Lop: 24T_DT1                          |\n");
...
void inMenu()
{
    ...
    printf("13. Xuat hang hoa\n");
    printf("14. Them vat tu vao cuoi danh sach\n");
    printf("\n\n");
    setColor(1);
    printf("========================================================================================================================\n\n");
    setColor(10);
