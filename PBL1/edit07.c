...
void docFile(VatTu ds[], int *n)
{
    ...
    if (*n + m > MAX_VAT_TU)
    {
        printf("So luong vat tu vuot qua gioi han\n");
        fclose(f);
        return;
    }
    int i = *n;
    *n += m;
    for (; i < *n; i++)
    {
        if (fscanf(f, "%s", ds[i].ma_vat_tu) != 1 ||
            fscanf(f, " %[^\n]", ds[i].ten_vat_tu) != 1 ||
            fscanf(f, " %[^\n]", ds[i].loai_vat_tu) != 1 ||
            fscanf(f, "%s", ds[i].don_vi_tinh) != 1 ||
        ...
        {
            printf("Xay ra loi khi doc du lieu tu filefile.\n");
            fclose(f);
            *n = i;
            return;
        }
...
{
    printf("Ma vat tu: ");
    scanf("%s", vt->ma_vat_tu);
    printf("Ten vat tu: ");
    scanf(" %[^\n]", vt->ten_vat_tu);
    printf("Loai vat tu: ");
    scanf(" %[^\n]", vt->loai_vat_tu);
    printf("Don vi tinh: ");
    scanf("%s", vt->don_vi_tinh);
...
void chenVatTuVaSapXep(VatTu ds[], int *n, int sapxeptheo)
{
    int i;
    printf("Nhap thong tin vat tu can chen:\n");
    nhapMotVatTu(&ds[*n]);
    ...
    for (i = *n; i > 0; i--)
    {
        if (soSanhVatTu(ds[i - 1], ds[i], sapxeptheo))
        {
...
int main()
{
    ...
    do
    {
        ...
        case 10:
            ...
            break;
        case 11:
            chenVatTuVaSapXep(danhSach, &n, sapxeptheo);
            break;
        default:
