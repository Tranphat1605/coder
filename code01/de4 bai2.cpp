...
void nhapDanhSach(VatTu ds[], int *n) {
    ...
    for (int i = 0; i < *n; i++) {
        ...
        getchar(); // Xóa bộ đệm trước khi nhập chuỗi
...
void xoaVatTu(VatTu ds[], int *n) {
    for (int i = 0; i < *n; i++) {
        if (ds[i].so_luong > 100) {
            ...
            (*n)--;
            i--; // Lùi lại để kiểm tra phần tử mới dời lên
        }
    }
}
...
void thongKeTheoLoai(VatTu ds[], int n) {
    typedef struct {
        char loai_vat_tu[20];
        int tong_so_luong;
        float tong_thanh_tien;
    } ThongKe;

    ThongKe tk[MAX_VAT_TU];
    int soLoai = 0;

    for (int i = 0; i < n; i++) {
        int found = 0;
        for (int j = 0; j < soLoai; j++) {
            if (strcmp(tk[j].loai_vat_tu, ds[i].loai_vat_tu) == 0) {
                tk[j].tong_so_luong += ds[i].so_luong;
                tk[j].tong_thanh_tien += ds[i].thanh_tien;
                found = 1;
                break;
            }
        }
        if (!found) {
            strcpy(tk[soLoai].loai_vat_tu, ds[i].loai_vat_tu);
            tk[soLoai].tong_so_luong = ds[i].so_luong;
            tk[soLoai].tong_thanh_tien = ds[i].thanh_tien;
            soLoai++;
        }
    }
    ...
    for (int i = 0; i < soLoai; i++) {
        printf("%-20s %-10d %-10.2f\n", tk[i].loai_vat_tu, tk[i].tong_so_luong, tk[i].tong_thanh_tien);
    }
}
